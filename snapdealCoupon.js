...
function postProcessor(coupon, i){
    ...
    if(savings > $('.valuetag-sav-amt').text()){
    var currentSavAmt = parseFloat($('.valuetag-sav-amt').text()),
        finalSavAmt = savings;
...
function temp(coupon, i, lenArray){
  if(lenArray==100){
    $('.valuetag-loading').html('Automatically applying the best coupon now !');
    $('.valuetag-lb-progress').text("100% Complete");
    $('.valuetag-lb-fg').css("width", "100%");
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=snapdeal",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
