...
function preProcessor(i, coupon){
    //$('#couponRemove').click();
    $('#spc-gcpromoinput').removeAttr("disabled");
    $('.a-button-inner input:eq(0)').removeAttr("disabled");
    $('#spc-gcpromoinput').val(coupon);
   $('.a-button-inner input:eq(0)').click();
...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('#couponRemove')[0]))    $('#couponRemove')[0].click();
            $('#spc-gcpromoinput').removeAttr("disabled");      $('.a-button-inner input:eq(0)').removeAttr("disabled");
            $('#spc-gcpromoinput').val(coup_req);                 $('.a-button-inner input:eq(0)').click();
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    $('a.prmocode').click();
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=amazon-us",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('amazon').length>1){
        var imgURL = chrome.extension.getURL("valtick128.png");
        console.log("TEst passed", document.getElementById("couponClick") );
        if($('#spc-gcpromoinput').length>0  && !document.getElementById("couponClick")){
            $('#spc-top:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
            addToDOM();
