...
function preProcessor(i, coupon){
    //$('#couponRemove').click();
    $('#enterCoupon').removeAttr("disabled");
...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            $('#enterCoupon').val(coup_req);                $('#couponApply').click();//Put coupon value and click
            flagCoupon[0] = 2;
...
function couponCheck(){
    ...
    if(curURL.split('cartId=').length>1){ //Validate checkout page
        var imgURL = chrome.extension.getURL("valtick128.png");
        console.log("TEst passed", document.getElementById("couponClick") );
        if($('#enterCoupon').length>0  && !document.getElementById("couponClick")){//Validate coupon text box
            $('#roCouponTable:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>"); //Put auto apply icon
            addToDOM();
