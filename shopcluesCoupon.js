...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('#couponRemove')[0]))    $('#couponRemove')[0].click();
            $('#enterCoupon').removeAttr("disabled");       $('#couponApply').removeAttr("disabled");
            $('#enterCoupon').val(coup_req);                  $('#couponApply').click();
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=shopclues",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('dispatch=checkout').length>1){
        var imgURL = chrome.extension.getURL("valtick128.png");
        console.log("TEst passed", document.getElementById("couponClick") );
        if($('#coupon_field').length>0  && !document.getElementById("couponClick")){
            $('.box_paymentcalculations:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
            addToDOM();
