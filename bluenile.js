...
function postProcessor(coupon, i){
    ...
    oldtime1 = newtime1;
      ...
      savings = 0;
      for(k=0;k<1;k++){
          var amount = $('.total').text().split('$')[1];
          savings = initial-parseFloat(amount);
          console.log($('#summaryValue').text(), $('summaryValue').text().split('$'), amount, savings);
      }
   ...
   // $('.remove')[3].removeAttr("disabled");
    ...
    if(!/undefined/.test($('.remove')[3]))    $('.remove')[3].click();//Validate coupon remove functionality or comment it
...
function preProcessor(i, coupon){
    //$('#couponRemove').click();
    $('.promo-code-input input:eq(0)').removeAttr("disabled");
    $('.promo-code-form a:eq(0)').removeAttr("disabled");
    $('.promo-code-input input:eq(0)').val(coupon);
    $('.promo-code-form a:eq(0)').click(); 
...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('.remove')[3]))    $('.remove')[3].click();//Validate coupon remove functionality or comment it
        $('.promo-code-input input:eq(0)').removeAttr("disabled");      $('.promo-code-form a:eq(0)').removeAttr("disabled");
        $('.promo-code-input input:eq(0)').val(coupon);                 $('.promo-code-form a:eq(0)').click();//Put coupon value and click
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=bluenile",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('bluenile').length>1){ //Validate checkout page
        var imgURL = chrome.extension.getURL("valtick128.png");
        console.log("TEst passed", document.getElementById("couponClick") );
        if($('.promo-code-input').length>0  && !document.getElementById("couponClick")){//Validate coupon text box
            $('.promo-code-input:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>"); //Put auto apply icon
            addToDOM();
            console.log('yay');
            initial= parseFloat($('.total').text().split('$')[1]);
            console.log(initial);
