...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('#couponRemove')[0]))    $('#couponRemove')[0].click();//Validate coupon remove functionality or comment it
            $('#promoCode').removeAttr("disabled");       $('#promoBtn').removeAttr("disabled");
            $('#promoCode').val(coup_req);                $('#promoBtn').click();//Put coupon value and click
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=officemax",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
