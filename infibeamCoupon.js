...
function postProcessor(coupon, i){
    if($('#promo-code-error').text()!=""){
      ...
      savings = 0;
      for(k=0;k<1;k++){
          var amount = $('.promo-names-summary.amount:eq(' + k + ')').text().replace(/[^0-9]/g, '');
          savings = savings + parseFloat(amount);
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=infibeam",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('ShowCart.action').length>1){
        var imgURL = chrome.extension.getURL("valtick128.png");
        console.log("TEst passed");
        if($('.submitCheckout').length>0){
            $('.promo-names:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
            addToDOM();
            ...
            setTimeout(function(){couponCheck();},4000);
        }
    }
}
