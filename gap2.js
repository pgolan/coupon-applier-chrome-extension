...
coupArray = [];
bestCouponFound = 0;
var newtime, oldtime, newtime1, oldtime1,initial;
...
function postProcessor(coupon, i){
    ...
    oldtime1 = newtime1;
      ...
      console.log('else');
      for(k=0;k<1;k++){
          var amount = $('#orderSummaryTotal').text().split('$')[1];

          savings = initial - parseFloat(amount);
          console.log($('#orderSummaryTotal').text(), $('#orderSummaryTotal').text().split('$'), amount, savings,initial);
    ...
    $('.removePromoLink').removeAttr("disabled");
    ...
    if(!/undefined/.test($('.removePromoLink')[0]))    $('.removePromoLink')[0].click();//Validate coupon remove functionality or comment it
...
function preProcessor(i, coupon){
    //$('#couponRemove').click();
    $('#billingInputPromoRewardCode').removeAttr("disabled");
    $('#billingPromoCodeApply').removeAttr("disabled");
    $('#billingInputPromoRewardCode').val(coupon);
    $('#billingPromoCodeApply').click(); 
...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('.removePromoLink')[0]))    $('.removePromoLink')[0].click();//Validate coupon remove functionality or comment it
            $('#billingInputPromoRewardCode').removeAttr("disabled");       $('#billingPromoCodeApply').removeAttr("disabled");
            $('#billingInputPromoRewardCode').val(coup_req);                $('#billingPromoCodeApply').click();//Put coupon value and click
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    var at=$('#orderSummaryTotal').text().split('$')[1];
    initial=parseFloat(at);
            console.log($('#orderSummaryTotal').text().split('$')[1],initial);

    bestCouponFound = 0;
    $('.valuetag-discount-cover:eq(0)').css("display", "block");
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=gap",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('checkout').length>1){ //Validate checkout page
        var imgURL = chrome.extension.getURL("valtick128.png");
        console.log("TEst passed", document.getElementById("couponClick") );
        if($('#billingInputPromoRewardCode').length>0  && !document.getElementById("couponClick")){//Validate coupon text box
            $('#billingPromoCodeInput:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>"); //Put auto apply icon
            addToDOM();
            console.log('yay');
