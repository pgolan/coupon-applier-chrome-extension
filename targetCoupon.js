...
function getCoupons(){
    ...
    $('a.prmocode').click();
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=target",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('checkout').length>1){
        ...
        if($('a.prmocode').length>0  && !document.getElementById("couponClick")){
            $('div.widgets_sec:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
            addToDOM();
