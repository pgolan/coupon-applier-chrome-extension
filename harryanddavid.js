...
function postProcessor(coupon, i){
    ...
    if($('#ajax_error').css('display') != 'none'){
      ...
      savings = 0;
      for(k=0;k<1;k++){
          var amount = $('.line coupons clearfix').text().split('$')[1];
          savings = parseFloat(amount);
          console.log($('.line coupons clearfix').text(), $('.line coupons clearfix').text().split('$'), amount, savings);
      }
    }
    ...
    $('#BTN_PROMOCODE_REMOVE').removeAttr("disabled");
    ...
    if(!/undefined/.test($('#BTN_PROMOCODE_REMOVE')[0]))    $('#BTN_PROMOCODE_REMOVE')[0].click();//Validate coupon remove functionality or comment it
...
function preProcessor(i, coupon){
    ...
    $('#couponSubmit_1').removeAttr("disabled");
    $('#promoCode').val(coupon);
    $('#couponSubmit_1').click(); 
...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('#couponRemove')[0]))    $('#couponRemove')[0].click();//Validate coupon remove functionality or comment it
            $('#promoCode').removeAttr("disabled");       $('#couponSubmit_1').removeAttr("disabled");
            $('#promoCode').val(coup_req);                $('#couponSubmit_1').click();//Put coupon value and click
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=harryanddavid",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    console.log("CP Check was called");
     //Validate checkout page
        ...
        if($('#promoCode').length>0  && !document.getElementById("couponClick")){//Validate coupon text box
            $('#promotionSection:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>"); //Put auto apply icon
            addToDOM();
            console.log('yay');
