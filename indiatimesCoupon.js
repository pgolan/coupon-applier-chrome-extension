...
function postProcessor(coupon, i){
    if($('#checkGCError').text()!=""){
      savings = 0;
    }
    else {
        savings = 0;
        if( $('.gccalculator_txt1_redeemed').css('display') != 'none' ){
            for(k=0;k<1;k++){
              var amount = $('#totalDiscount:eq(' + k + ')').text().replace(/[^0-9\.]/g, '');
    ...
    if(savings > parseFloat($('.hdc-sav-amt').text())){
...
function temp(coupon, i, lenArray){
  if(lenArray==100){
    ...
    setTimeout(function(){temp(coupon, i, lenArray);},4500);
  }
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=indiatimes",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('/p_').length>1){
        var imgURL = chrome.extension.getURL("valtick128.png");
        console.log("TEst passed", document.getElementById("couponClick") );
        if($('#cartitems').length>0  && !document.getElementById("couponClick")){
            $('.cartitemsrow:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
            addToDOM();
