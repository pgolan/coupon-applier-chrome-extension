...
function postProcessor(coupon, i){
    ...
    if($('#ajax_error').css('display') != 'none'){
      ...
      savings = 0;
      for(k=0;k<1;k++){
          var amount = $('#summaryValue').text().split('$')[1];
...
function preProcessor(i, coupon){
    ...
    $('#lnkApplyPromos').removeAttr("disabled");
    $('#promoCode').val(coupon);
    $('#lnkApplyPromos').click(); 
...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('#couponRemove')[0]))    $('#couponRemove')[0].click();//Validate coupon remove functionality or comment it
            $('#promoCode').removeAttr("disabled");       $('#lnkApplyPromos').removeAttr("disabled");
            $('#promoCode').val(coup_req);                $('#lnkApplyPromos').click();//Put coupon value and click
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=bloomingdales",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('bag').length>1){ //Validate checkout page
        ...
        if($('#promoCode').length>0  && !document.getElementById("couponClick")){//Validate coupon text box
            $('#promoCodeContainer:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>"); //Put auto apply icon
            addToDOM();
            console.log('yay');
