...
function postProcessor(coupon, i){
    ...
    if($('#ajax_error').css('display') != 'none'){
      ...
      savings = 0;
      for(k=0;k<1;k++){
          var amount = $('#totalSavingsTip').text().split('-INR')[1];
          savings = parseFloat(amount);
          console.log($('#totalSavingsTip').text(), $('#totalSavingsTip').text().split('-INR'), amount, savings);
      }
    }
    ...
    if(!/undefined/.test($('.arw_lft')[0]))    $('.arw_lft')[0].click();//Validate coupon remove functionality or comment it
...
function preProcessor(i, coupon){
    //$('#couponRemove').click();
    $('#dc_redeem_code').removeAttr("disabled");
    $('.button blk_btn')[0].removeAttr("disabled");
    $('#dc_redeem_code').val(coupon);
    $('.button blk_btn')[0].click(); 
...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('.arw_lft')[0]))    $('.arw_lft')[0].click();//Validate coupon remove functionality or comment it
            $('#dc_redeem_code').removeAttr("disabled");       $('.button blk_btn')[0].removeAttr("disabled");
            $('#dc_redeem_code').val(coup_req);                $('.button blk_btn')[0].click();//Put coupon value and click
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=lulus",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('cart').length>1){ //Validate checkout page
        ...
        if($('#dc_redeem_code').length>0  && !document.getElementById("couponClick")){//Validate coupon text box
            $('#coupons_credits:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>"); //Put auto apply icon
            addToDOM();
            console.log('yay');
