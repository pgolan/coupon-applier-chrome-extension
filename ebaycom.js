...
function preProcessor(i, coupon){
    //$('#couponRemove').click();
    $('#xocpnety_cnt').removeAttr("disabled");
    $('#but_aplycpn').removeAttr("disabled");
    $('#xocpnety_cnt').val(coupon);
    $('#but_aplycpn').click(); 
...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('#couponRemove')[0]))    $('#couponRemove')[0].click();//Validate coupon remove functionality or comment it
            $('#xocpnety_cnt').removeAttr("disabled");       $('#but_aplycpn').removeAttr("disabled");
            $('#xocpnety_cnt').val(coup_req);                $('#but_aplycpn').click();//Put coupon value and click
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=ebay",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('checkout').length>1){ //Validate checkout page
        ...
        if($('#xocpnety_cnt').length>0  && !document.getElementById("couponClick")){//Validate coupon text box
            $('#xocsmry:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>"); //Put auto apply icon
            addToDOM();
