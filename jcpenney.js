...
function preProcessor(i, coupon){
    ...
    $('#promoSubmit').removeAttr("disabled");
...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('.arw_lft')[0]))    $('.arw_lft')[0].click();//Validate coupon remove functionality or comment it
            $('#promocode').removeAttr("disabled");       $('#promoSubmit').removeAttr("disabled");
            $('#promocode').val(coup_req);                $('#promoSubmit').click();//Put coupon value and click
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=jcpenney",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('cart').length>1){ //Validate checkout page
        ...
        if($('#promocode').length>0  && !document.getElementById("couponClick")){//Validate coupon text box
            $('#couponCodeInfo:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>"); //Put auto apply icon
            addToDOM();
            console.log('yay');
