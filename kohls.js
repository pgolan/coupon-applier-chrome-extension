...
function postProcessor(coupon, i){
    ...
    oldtime1 = newtime1;
    savings = 0;
    if($('.pgreen').text().split('$')[1]!=null)
    {
      
      for(k=0;k<1;k++){
          var amount = $('.pgreen').text().split('$')[1];
          savings = parseFloat(amount);
          console.log($('.pgreen').text(), $('.pgreen').text().split('$'), amount, savings);
      }
    }

    if(savings > parseFloat($('.pgreen:eq(0)').text().split('$'))){
    var currentSavAmt = parseFloat($('.pgreen:eq(0)').text().split('$')),
        finalSavAmt = savings;
       $({c: currentSavAmt}).animate({c: finalSavAmt}, {
            step: function(now) {
                $('.pgreen:eq(0)').text(Math.round(now))
            },
            duration: 1000,
            easing: "linear"
        });
    ...
    $('.removeAppliedDiscount r_link').removeAttr("disabled");
    console.log('couponRemove', i, ": Savings for " + coupon + " is " + savings, ' :: old savings ', $('.pgreen:eq(0)').text(), ':', parseFloat($('.pgreen:eq(0)').text().split('$')));
    if(!/undefined/.test($('.removeAppliedDiscount r_link')[0]))    $('.removeAppliedDiscount r_link')[0].click();//Validate coupon remove functionality or comment it
...
function preProcessor(i, coupon){
    //$('#couponRemove').click();
    $('#apply_code_input').removeAttr("disabled");
    $('.applyCodeButton').removeAttr("disabled");
    $('#apply_code_input').val(coupon);
...
function endProcess(i){
    ...
    if(flagCoupon[i]==0){
        ...
        if(max>0){
            ...
            if(!/undefined/.test($('.removeAppliedDiscount r_link')[0]))    $('.removeAppliedDiscount r_link')[0].click();//Validate coupon remove functionality or comment it
            $('#apply_code_input').removeAttr("disabled");       $('.applyCodeButton').removeAttr("disabled");
            $('#apply_code_input').val(coup_req);                $('.applyCodeButton').click();//Put coupon value and click
            flagCoupon[0] = 2;
...
function getCoupons(){
    ...
    $.ajax({
        url: "http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=kohls",
        type: "POST",
        dataType: 'json',
        processData: false
    }).done(function (data) {
...
function couponCheck(){
    ...
    if(curURL.split('checkout').length>1){ //Validate checkout page
        ...
        if($('#apply_code_input').length>0  && !document.getElementById("couponClick")){//Validate coupon text box
            $('#apply_code_input:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>"); //Put auto apply icon
            addToDOM();
